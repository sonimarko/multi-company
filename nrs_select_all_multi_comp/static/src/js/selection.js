odoo.define('nrs_select_all_multi_comp.SwitchCompanyMenu', function(require) {
    "use strict";
    ...
    var SwitchCompanyMenu = Widget.extend({
        ...
        events: {
            ...
            'click .btn-select-all': '_onClickSelectAll',
            'click .btn-deselect-all': '_onClickDeselectAll',
        },
        ...
        willStart: function () {
            ...
            this.operating_countries_company = session.operating_country_companies;
            this.admin_company = session.admin_companies;

            this.is_select_all_active = session.is_select_all_active;
            this.is_deselect_all_active = session.is_deselect_all_active;
            this.is_operation_country_active = session.is_operation_country_active;
            this.is_admin_company_active = session.is_admin_company_active;
         ...
         */
         _onClickSelectAll: function (ev) {
            ...
            ev.preventDefault();
            ev.stopPropagation();
            var dropdownMenu = $(ev.currentTarget).parent();
            var allowed_company_ids = this.allowed_company_ids;
            var current_company_id = allowed_company_ids[0];

            var company_items = dropdownMenu.find('.dropdown-item');
            var length = company_items.length;

            for (let i = 0; i < length; i++) {
                var current_comp = $(company_items[i]);
                var companyID = current_comp.data('company-id');
                
                if (current_comp.find('.fa-square-o').length) {
                    allowed_company_ids.push(companyID);
                    current_comp.find('.fa-square-o').removeClass('fa-square-o').addClass('fa-check-square');
                    current_comp.attr('aria-checked', 'true');
                }
            }
         ...
         * @param {MouseEvent|KeyEvent} ev
         */
        _onClickDeselectAll: function (ev) {
            if (ev.type == 'keydown' && ev.which != $.ui.keyCode.ENTER && ev.which != $.ui.keyCode.SPACE) {
                return;
            }
            ev.preventDefault();
            ...
            for (let i = 0; i < length; i++) {
                ...
                if (current_comp.find('.fa-check-square').length) {
                    allowed_company_ids.splice(allowed_company_ids.indexOf(companyID), 1);
                    current_comp.find('.fa-check-square').removeClass('fa-check-square').addClass('fa-square-o');
                    current_comp.attr('aria-checked', 'false');
                }
            }
